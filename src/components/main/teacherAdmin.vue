<template>
    <div>
        <el-row>
            <el-col :span="24">
                <el-card>
                    <el-select v-model="selectIndex" placeholder="请选择查询字段" style="width: 30%;font-size:10px">
                        <el-option label="工号" value="userid"></el-option>
                    ...
                    <el-table
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                <el-button
                                        size="mini"
                                        @click="cshow(scope.$index, scope.row)">编辑</el-button>
                                <el-button
                                        size="mini"
                                        type="danger"
        ...
        methods: {
                ...
                this.$axiosjwt({
                    ...
                    success: (result) => {
                        ...
                        for(var i = 0;i < this.tableData.length;i ++){
                            this.tableData[i].classes = this.tableData[i].classes.split(';')
                        }
                        this.UserSize = result.total
                    }
                })
            ...
            handleClose(item,index){
              console.log(item)
                ...
                this.$confirm("将要免除"+ this.tableData[index].username + "在" + item + "中教师职务","提示",{
                    confirmButtonText:"确定",
                    cancelButtonText:'取消',
                    type:'warning'
                }).then(()=>{

                    this.tableData[index].classes.splice(this.tableData[index].classes.indexOf(item), 1);

                    let newDuties = this.tableData[index].classes.join(';')

                    let u_id = this.tableData[index].id
                    this.$axiosjwt({
                        url:'/admin/upClassManage',
                        method:'post',
                        data:{
                            newDuties,
                            u_id,
            ...
            handleSizeChange(val) {
                this.pageSize = val;
                //当输入框等不为空搜索,否则全部分页显示
                if (this.input && this.selectIndex) {
                    this.select()
                } else {
                    this.get(this.pageSize, this.pageNo)
                }
